...
public class Mesh {
	...
	/**
	 * Construct a new Mesh from parameters. The passed in objects actually back the Mesh.
	 * @param vertices All the vertices that make up the mesh.
	 * @param triangles An array indexing into the vertices array specifying triangles. v1,v2,v3,v1,v2,v3
	...
	public Mesh getScaledInstance(float scale) {
		...
		return new Mesh(scaledVertices, Arrays.copyOf(uvs, uvs.length), Arrays.copyOf(triangles, triangles.length));
	}
	
	public Mesh clone() {
		return new Mesh(Arrays.copyOf(vertices,  vertices.length), 
						isUvMapped() ? Arrays.copyOf(uvs, uvs.length) : null, 
						Arrays.copyOf(triangles, triangles.length));
	}
